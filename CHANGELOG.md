...
- Removed artificial promotional badges and toned down marketing claims across documentation.
- Removed unused legacy template directories (`training/`, `marketplace/`, `videos/`) to streamline the repository.
- Eliminated decorative emojis from headings and guides for a clean, professional open-source presentation.
- Configured documentation deployment in CI to publish independently on pushes to main.
...
- Replaced promotional PM2 comparisons with neutral migration guidance.
- Updated package and documentation release metadata for npm publication.

### Fixes

- Preserved the `bs9` CLI binary mapping in the published npm package.
- Made the session TTL test reliable on slower CI runners.
